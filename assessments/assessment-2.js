    ...
    removeGood(id) {
        ...
        for (let i = 0; i < this.goods.length; i++) {
            if (this.goods[i].id !== id) {
                newGoodsArray.push(this.goods[i]);
            }
        }
    ...
    searchGoodByName(name) {
        ...
        for (let i = 0; i < this.goods.length; i++) {
            if (this.goods[i].name === name) {
                ...
                // return this.goods[i];     this works for only one(first) good. But we have to return all of goods that matched with name.
            }
        }
        return matchedGoods;
    }
...
const apple1 = new Good(1, "Apple", 50, 0.5);
const apple2 = new Good(4, "Apple", 30, 1.2);
